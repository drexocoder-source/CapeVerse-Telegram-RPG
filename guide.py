...
def ensure_guide_pdf() -> Path:
    ...
    story = [
        ...
        Paragraph("/start → create or reopen an account<br/>/main → open the game menu<br/>/inventory → view wallet, collection, relics, research, and rewards<br/>/team → manage, name, and activate Team 1, Team 2, or Team 3<br/>/pve → normal enemies and villain hunts<br/>/pvp → asynchronous player Arena<br/>/profile → generate a visual player dossier<br/>/char NAME → global codex photo, story, moves, and research/evolution<br/>/mychar NAME → owned character level, XP, stars, and actions<br/>/daily and /weekly → timed rewards<br/>/guide → guide center", body),
        ...
        Paragraph("Teams and battle entry", heading),
        Paragraph("Every player may build and name three separate teams. Each lineup accepts up to three owned characters. Select one active team for PvE, PvP Arena, Rift, and events. Use /pve for enemy operations and /pvp for asynchronous battles against another player’s defensive signal.", body),
