...
def ensure_guide_pdf() -> Path:
    ...
    story = [
        ...
        Paragraph("/start → create or reopen an account<br/>/main → open the main game menu<br/>/menu → open the main game menu<br/>/profile → generate a visual player card<br/>/char → browse owned characters<br/>/char NAME → search full character moves, XP, unlocks, and evolution<br/>/guide → open the guide center<br/>/help → return to the main menu", body),
        ...
        Paragraph("Moves unlock at their configured character level. Locked moves are visible in /char but do not appear in battle. Character evolution requires level 10 and 3 stars; use the evolution button shown in /char.", body),
        ...
        Paragraph("The hero and villain wizards ask one question at a time. After the story, choose AI generation or manual move entry. AI generation uses the description to propose original balanced Normal, Defense, and Special move lists. It also proposes villain difficulty data. AI output remains a draft and still requires owner rights review and approval.", body),
        Paragraph("Manual move format: one move per line as <b>Name | Description | Damage | Unlock level | Cooldown | Effect</b>. Each category accepts 1 to 3 moves. Villains also receive minimum and maximum player levels; their health and damage scale upward for eligible higher-level players.", body),
